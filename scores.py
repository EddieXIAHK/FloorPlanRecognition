...
def evaluate_semantic(benchmark_path, result_dir, num_of_classes=11, need_merge_result=False, im_downsample=False, gt_downsample=False):
	...
	for i in range(n):
		im = imageio.imread(im_paths[i])
		if need_merge_result:
			im_d = imageio.imread(im_d_paths[i], mode='L')
			im_cw = imageio.imread(im_cw_paths[i], mode='L')
		# create fuse semantic label
		cw = imageio.imread(cw_paths[i])
		if len(cw.shape) == 3:
			cw = cw[:, :, 0]  # Convert to grayscale if RGB
		dd = imageio.imread(d_paths[i])
		if len(dd.shape) == 3:
			dd = dd[:, :, 0]  # Convert to grayscale if RGB
		rr = imageio.imread(r_paths[i])

		if im_downsample:
			im = (skresize(im, (512, 512), preserve_range=True, anti_aliasing=True)).astype(np.uint8)
			if need_merge_result:
				im_d = (skresize(im_d, (512, 512), preserve_range=True, anti_aliasing=True)).astype(np.uint8)
				im_cw = (skresize(im_cw, (512, 512), preserve_range=True, anti_aliasing=True)).astype(np.uint8)
				im_d = im_d / 255
				im_cw = im_cw / 255

		if gt_downsample:
			cw = (skresize(cw, (512, 512), preserve_range=True, anti_aliasing=True)).astype(np.uint8)
			dd = (skresize(dd, (512, 512), preserve_range=True, anti_aliasing=True)).astype(np.uint8)
